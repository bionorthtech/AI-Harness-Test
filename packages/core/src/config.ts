...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["opencode.json", "opencode.jsonc", "bridle.json", "bridle.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".opencode", ".bridle", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => [".opencode", ".bridle"].includes(path.basename(item)))
        .toReversed()
        .map((directory) => AbsolutePath.make(directory)),
    ]
    // A config closer to the opened directory should win over one higher up.
    // Search starts nearby, so reverse the results before applying them.
    const directPaths = discovered.filter((item) => ![".opencode", ".bridle"].includes(path.basename(item))).toReversed()
